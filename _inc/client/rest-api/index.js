...
function JetpackRestApiClient( apiRoot, apiNonce ) {
	const methods = {
		...
		.then( checkStatus ).then( response => response.json() ),
		getStatsData: ( range ) => fetch( `${ apiRoot }jetpack/v4/module/stats/get`, {
			method: 'put',
			credentials: 'same-origin',
			headers: {
				'X-WP-Nonce': apiNonce,
...
const restApi = new JetpackRestApiClient( window.Initial_State.WP_API_root, window.Initial_State.WP_API_nonce );
